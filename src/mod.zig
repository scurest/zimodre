...
pub fn parse(ctx: &Parser) -> %Mod {
    ...
    const magic = %return ctx.nextN(u8, 4, "magic"); // offset 0
    const unknown1 = %return ctx.nextN(u8, 2, "unknown1"); // 3
    const bone_count = %return ctx.next(u16, "bone_count"); // 6
    const mesh_count = %return ctx.next(u16, "mesh_count"); // 8
    const unknown2 = %return ctx.nextN(u8, 2, "unknown2"); // 10
    const vertex_count = %return ctx.next(u32, "vertex_count"); // 12
    const unknown3 = %return ctx.nextN(u8, 8, "unknown3"); // 16
    const vertex_size = %return ctx.next(u32, "vertex_size"); // 24
    const unknown4 = %return ctx.nextN(u8, 8, "unknown4"); // 28
    ...
    const unknown5 = %return ctx.nextN(u8, 2, "unknown5"); // 46
    ...
    { var i: usize = 0; while (i != mesh_count) : (i += 1) {
        const item = %return parseMeshInfo(ctx);
        %%meshes.append(&item);
    }}
    ctx.end();
...
fn parseMeshInfo(ctx: &Parser) -> %MeshInfo {
    ...
    const unknown4 = %return ctx.nextN(u8, 16, "unknown4"); // 32
    ctx.end();
